...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+064",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Engraver",						// *** Plugin name - make it different for each plugin 
...
// Based on shader source https://www.shadertoy.com/view/XlsXDN
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float amplitud = 0.03;
	float frecuencia = iMouse.z / iResolution.x* 20.0;
	float gris = 1.0;
	...
	for (int i = 0; i < kNumPatrones; i++)
	{
		...
		vec2 punto = (0.1 + iMouse.x / iResolution.x)* vec2(
		...
		float brillo = -0.5 + 0.7*iMouse.y / iResolution.y + 0.3 * color.r + 0.4 * color.g + 0.3 * color.b;
		...
		if (dist < grosor && brillo < 0.75 - 0.12 * float(i))
		{
			...
			float fx = iMouse.w / iResolution.y *abs((x - 0.5) / k) - (0.5 - k) / k;
			gris = min(fx, gris);
		}
	}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Frequency",       FF_TYPE_STANDARD, 0.2f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Threshold",       FF_TYPE_STANDARD, 0.8f); m_UserMouseY = 0.7f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Curvature ",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Harder/Softer",  FF_TYPE_STANDARD, 0.8f); m_UserMouseLeftY = 1.0f;
