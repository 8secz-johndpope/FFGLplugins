...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+070",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Fake3DLines",						// *** Plugin name - make it different for each plugin 
...
const int height = 5;
float bars = 60.* iMouse.x / iResolution.x;
float textureIn(vec2 uv) {
	uv.y += float(height) / (1.5*bars);
	return(texture(iChannel0, uv).r + texture(iChannel0, uv).g + texture(iChannel0, uv).b) / 3.;
}
float draw(vec2 uv) {
	float thickness = .7*(0.01 + iMouse.y / iResolution.y);
	//return abs(textureIn(vec2(uv.x,floor(uv.y*bars)/bars))-fract(uv.y*bars))<0.1?1.:0.;
	float o = 0.;
	for (int i = 0; i<height; i++) {
		o += abs((textureIn(vec2(uv.x, (floor(uv.y*bars) - float(i)) / bars))*float(height)) - (fract(uv.y*bars) + float(i)))<thickness ? 1. : 0.;
	}
	return o;
}
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
	...
	for (int x = -2; x <= 2; x++) {
		for (int y = -2; y <= 2; y++) {
			o += draw(uv + (vec2(x, y) / (iResolution.xy*5.)));
		}
	}
	o /= 25.;
	fragColor = vec4(mix(vec3(0.1), vec3(1., uv.y, .5 - uv.y), o), 1.);
}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Bars",       FF_TYPE_STANDARD, 0.8f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Line width",       FF_TYPE_STANDARD, 0.1f); m_UserMouseY = 0.5f;
/*	SetParamInfo(FFPARAM_MOUSELEFTX,    "Spiral ",  FF_TYPE_STANDARD, 0.25f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Polar",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftY = 0.5f;
