...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+036",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+StreakyBlobs",						// *** Plugin name - make it different for each plugin 
...
// forked from http://glslsandbox.com/e#96.0

float f(vec3 o)
{
	float a = (2.*(.5 -inputVector1.z)*sin(o.x) + 2.*(.5-inputVector1.y)*o.y)*2.352; // o.y*0.25

	o = vec3(sin(iTime + a)*sin(o.x + o.z / 3.0)*o.x,
		sin(iTime + a)*cos(o.y + o.z / 2.0)*o.y, o.z);
	return dot(cos(o)*cos(o), vec3(1)) - 1.2;
}

vec3 s(vec3 o, vec3 d)
{
	float t = -3.0 + 6.*inputVector1.x; //depth
	float dt = 0.2;
	float nh = 0.0;
	float lh = 0.0;
	for (int i = 0; i < 36; i++) //def i=50
	{
		nh = f(o + d * t);
		if (nh > 0.0) { lh = nh; t += dt; }
	}

	if (nh > 0.0) return vec3(0.0, 0.0, 0.0);

	t = (inputColour.w +0.01)*t - dt * nh / (nh - lh); // threshold

	vec3 e = vec3(-.5, .0, 0.0);
	vec3 p = o + d * t * inputColour.z; // d*t =stripes
	vec3 n = -normalize(vec3(f(p + e), f(p + e.yxy), f(p + e.yyx)) + vec3((sin(p*75.)))*.0001);

	return vec3(mix(((max(-dot(n, vec3(0.977)), 0.) + 0.125*max(-dot(n, vec3(-.707, -.707, 0)), 0.)))*(mod

	(length(p.xy)*5., 2.) < 1.0 ? vec3(1., 1., 1.) : vec3(.0, .0, .0))
		, vec3(1.0, 1.0, 1.2), vec3(pow(t / 10., 2.5))));
}
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	vec2 p = (0.05 + iMouse.x / iResolution.x)* 5.0 * vec2(fragCoord.xy - 0.5*iResolution.xy);//FOV
	p = p.xy / iResolution.y;
	p.y -= 6.*(iMouse.y / iResolution.y - .5);//ymove
	p.x -= 6.*(iMouse.z / iResolution.x - .5);//xmove
	p.y *= 6.*(.08 + inputColour.x); //stretch Y
	p.x *= 6.*(.08 + inputColour.y);//stretch x
	fragColor = vec4(s(vec3(.0, 10.0*(.5 - inputVector1.w), iTime + 20.*(iMouse.w / iResolution.y - 0.5)), //move forward/rear
		normalize(vec3(p.xy, sin(iTime / 2.0)*0.25 + 0.85))), 1.0);
}
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Cam FOV",       FF_TYPE_STANDARD, 0.2f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Cam Y",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Cam X",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Move Z",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Stretch Y",           FF_TYPE_STANDARD, 0.2f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "Stretch X",         FF_TYPE_STANDARD, 0.2f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "Stripes",          FF_TYPE_STANDARD, 1.0f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "ThreshTexture",         FF_TYPE_STANDARD, 1.0f); m_UserAlpha = 1.0f;



	SetParamInfo(FFPARAM_VECTOR1_X, "DepthLev", FF_TYPE_STANDARD, 0.5f);
	SetParamInfo(FFPARAM_VECTOR1_Y, "Grid/Rot Y", FF_TYPE_STANDARD, 0.6f);
	SetParamInfo(FFPARAM_VECTOR1_Z, "Grid/Rot X", FF_TYPE_STANDARD, 0.4f);
	SetParamInfo(FFPARAM_VECTOR1_W, "ShiftPos Y", FF_TYPE_STANDARD, 0.5f);
/*

	SetParamInfo(FFPARAM_VECTOR2_X, "ShiftPosX", FF_TYPE_STANDARD, 0.5f);
