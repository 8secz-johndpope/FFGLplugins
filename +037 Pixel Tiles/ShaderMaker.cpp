...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+037",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Pixel Tiles",						// *** Plugin name - make it different for each plugin 
...
{
	vec2 st = (fragCoord.xy*2. - iResolution.xy) / iResolution.y * .5;

	float cells = 40.*iMouse.x / iResolution.x;

	st = floor(st*cells) / cells;

	float w = iTime * sin(st.y * 2. * acos(-1.) * 4.);
	float v = (sin(2. * iTime + st.x * 2. * acos(-1.) * 8. + w) + 1.0) / 2.0;
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Size",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
