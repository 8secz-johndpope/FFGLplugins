...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+113",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Glitcher4",						// *** Plugin name - make it different for each plugin 
...
// Based on shader code by  boysx from https://www.shadertoy.com/view/MltBzf
float rand(vec2 p)
{
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec2 id = floor((.1 + iMouse.x / iResolution.x) *uv * 8.0);
	...
	float scanline = 3.*iMouse.y / iResolution.y*abs(cos(uv.y * 400.)); //scanline
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "ScanLines",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
/*	SetParamInfo(FFPARAM_MOUSELEFTX,    "Blockiness ",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "   ",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
