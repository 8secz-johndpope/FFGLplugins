...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+033",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D Liquidoil",						// *** Plugin name - make it different for each plugin 
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec2 r = (0.1 + iMouse.x / iResolution.x) *8.0*vec2(fragCoord.xy - 0.5*iResolution.xy) / iResolution.y;//zoom out
	
	vec2 q = vec2(p);
	float angle;
	angle = (2.*Pi)*(-0.5 + iMouse.w / iResolution.y); // angle in radians (PI is 180 degrees)

	 //rotation
	p.x = cos(angle)*r.x + sin(angle)*r.y;
	p.y = -sin(angle)*r.x + cos(angle)*r.y;

	//deformation 
      // p.x+= length(p * inputVector1.w);
	//	p.y+= length(p * inputVector2.x);

		
  for (int i = 1; i<complexity+int(10.*inputVector1.z) ; i++)
	{
		vec2 newp = p + iTime*0.1; //0.05
		newp.x += 1.0 / float(i)*sin(float(i)*p.y + iTime / fluid_speed + 0.3*float(i)) + 2.*Pi * iMouse.y / iResolution.y ;
		newp.y += 1.0 / float(i)*sin(float(i)*p.x + iTime / fluid_speed + 0.3*float(i)) + 2.*Pi * iMouse.z / iResolution.x ;
		p = newp;
	}
	vec3 col = vec3( inputColour.w* color_intensity*tan(0.901*p.x) + color_intensity - 0.5 + inputColour.x,
		inputVector1.x* color_intensity*tan(0.4*p.y) + color_intensity - 0.5 + inputColour.y,
		inputVector1.y* color_intensity*tan(p.x*0.8 + p.y) + color_intensity - 0.5 + inputColour.z);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "ZoomOut",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Shift X",       FF_TYPE_STANDARD, 0.25f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Shift Y",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Rotate",  FF_TYPE_STANDARD, 0.13f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "RedLevel",           FF_TYPE_STANDARD, 0.75f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "GreenLevel",         FF_TYPE_STANDARD, 0.5f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "BlueLevel",          FF_TYPE_STANDARD, 0.25f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "RedFill ",         FF_TYPE_STANDARD, 1.0f); m_UserAlpha = 1.0f;



	SetParamInfo(FFPARAM_VECTOR1_X, "GreenFill", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_Y, "BlueFill ", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_Z, "*Complexity ", FF_TYPE_STANDARD, 0.0f);
/*	SetParamInfo(FFPARAM_VECTOR1_W, "Deform1", FF_TYPE_STANDARD, 1.0f);


	SetParamInfo(FFPARAM_VECTOR2_X, "Deform2", FF_TYPE_STANDARD, 1.0f);
