...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+118",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+3D Metaball",						// *** Plugin name - make it different for each plugin 
...
const int MAX_STEPS = 64; //was 64
const float EPSILON = .0001;
const float STEP_SIZE = 0.975;
...
float map(in vec3 p) {
	return min(metaBalls(p), p.y + 1.2 * (2. * (1. - inputColour.y) - 0.5));//flood
}
...
vec3 shade(in vec3 ro, in vec3 rd, in float d) {
	...
	float mask = inputColour.x * smoothstep(1., .5, 30.*cos(50.*p.y) + sin(50.*p.x) + cos(50.*p.z));
	vec3 albedo = mix(vec3(.98), vec3(.01), mask); //color
	float metallic = 1.1- iMouse.z / iResolution.x; //lights
	float roughness = mix(.15, .75, mask);
	float ao = 0.01 + iMouse.w / iResolution.y;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
	...
	float t = iTime + 5.;
	float angle = radians(300. + 55. * t  + 355. * iMouse.y/iResolution.y);
	float dist = 1.25 + cos(1.5 * t);
	vec3 ro = vec3(dist * cos(angle), 2., dist * sin(angle));
	vec3 aim = vec3(.0);
	float zoom = 3.+3.*(.5- iMouse.x / iResolution.x); //ZOOMOUT
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Zoom Out",       FF_TYPE_STANDARD, 1.0f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Rotate",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Light",  FF_TYPE_STANDARD, 0.9f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Ambient",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Stripes",           FF_TYPE_STANDARD, 1.0f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "*Flood",         FF_TYPE_STANDARD, 0.0f); m_UserGreen = 0.5f;
/*	SetParamInfo(FFPARAM_BLUE,          "Angle",          FF_TYPE_STANDARD, 0.5f); m_UserBlue = 0.5f;
