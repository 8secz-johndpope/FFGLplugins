...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+088",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Cracked Glass",						// *** Plugin name - make it different for each plugin 
...
{
	vec2 p = 0.01 + 6.*iMouse.x / iResolution.x*(fragCoord.xy*2. - iResolution.xy) / iResolution.x;

	vec2 v = vec2(1E3);
	vec2 v2 = vec2(1E4);
	vec2 center = vec2(-1. + 2.*iMouse.yz / iResolution.yx); //(.1,-.2);
	for (int c = 0; c<90; c++)
	{
	...
	if (col<.3)col = 0.;
	vec4 tex = texture(iChannel0, (fragCoord.xy) / iResolution.xy + rnd(v)*.02);
...
//	SetParamInfo(FFPARAM_SPEED,         "Speed",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed = 0.5f;
	SetParamInfo(FFPARAM_MOUSEX,        "ZoomOut",       FF_TYPE_STANDARD, 0.2f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Left/Right",       FF_TYPE_STANDARD, 0.65f); m_UserMouseY = 0.5f;
