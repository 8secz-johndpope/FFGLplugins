...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+041",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+HyperMincer",						// *** Plugin name - make it different for each plugin 
...
{
	float t = trace(o, r);
	vec3 w = o + r * t + iMouse.w / iResolution.y;  //color loop
	vec3 sn = normal(w);

	float lit = occlusion(o, r) * 10.0; // lightness

	vec4 ac = surf(r, w, sn, t);

	vec3 from = vec3( inputColour.x, inputColour.y, inputColour.z); //color
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	uv = 10.* iMouse.x / iResolution.x * (uv * 2.0 - 1.0); //zoom
	uv.x *= iResolution.x / iResolution.y;
	uv.x -= 20.*(iMouse.y / iResolution.y-.5); //LEFT/Right
	uv.y -= 20.*(iMouse.z / iResolution.x-.5); //up-down
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSELEFTY,    "ColorLoop",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
