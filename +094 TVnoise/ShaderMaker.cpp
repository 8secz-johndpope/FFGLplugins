...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+094",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+TVnoise",						// *** Plugin name - make it different for each plugin 
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	// Snow effect
	float noiseIntensity = .95 *(iMouse.x / iResolution.x);
	float pixelDensity = 250. *iMouse.y / iResolution.y;
	...
	// Add a blue tint
	color.b += iMouse.w / iResolution.y*.2;

	// Vignette
	color *= vec3(1.0 - iMouse.z / iResolution.x*pow(distance(uv, vec2(0.5, 0.5)), 3.0) * 8.0);
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "NoiseIntense",       FF_TYPE_STANDARD, 1.0f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "ScaleNoise",       FF_TYPE_STANDARD, 1.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Vignette",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "BlueTint",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
