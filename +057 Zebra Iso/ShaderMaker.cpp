...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+057",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+ZebraIso",						// *** Plugin name - make it different for each plugin 
...
// based on shader code by lennyjpg from https://www.shadertoy.com/view/XscfzB

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{

	vec2 uv = fragCoord / iResolution.y; // Normalized pixel coordinates (from 0 to 1)



	float t = iTime*.5;

	vec2 center = vec2(iMouse.zx / iResolution.xy);

	float g = 6.*(.5 - iMouse.w / iResolution.y);

	center.x += sin(uv.y*g + t);
	center.y += cos(uv.x*g + t);
	float d = distance(uv, center);
	float k = iMouse.y / iResolution.y + sin(d*(0.06*iMouse.x + 1.5) + t);
	//    float k =iMouse.y/iResolution.y + sin(d*0.07*((iMouse.x+ 0.3))+ t);
	float e = smoothstep(0., fwidth(k)*0.9, k);

	fragColor = vec4(max(1. - e, 0.));
}
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "LineWidth",       FF_TYPE_STANDARD, 0.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Transform1",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Transform2",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftY = 0.5f;
