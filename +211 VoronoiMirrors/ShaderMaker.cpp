...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+211",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+VoronoiMirrors",						// *** Plugin name - make it different for each plugin 
...
// https://www.shadertoy.com/view/llKcDR 
// Fluid Breaking Mirror(voronoi)
...
vec3 calcNearestPoint2D(vec2 p) {
	...
	float index = -1.;
	for (float i = 0.; i < 12.*(.01 + iMouse.y / iResolution.y)*num_points; i++) {
...
vec2 calcOffsetOnI(float i) {
	return 2.*(.5 - iMouse.z / iResolution.x)*vec2(sin(i*point_spread), cos(i*speed*1.7));
}
float sq(float x) {
	return x * x;
}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "CellsAmount",       FF_TYPE_STANDARD, 0.7f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "AngleShift",  FF_TYPE_STANDARD, 0.7f); m_UserMouseLeftX = 0.5f;
