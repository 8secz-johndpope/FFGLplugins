...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+108",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+GlitchSat",						// *** Plugin name - make it different for each plugin 
...
// based on https://www.shadertoy.com/view/XtK3W3
//
...
// First corner
	...
	return iMouse.x / iResolution.x* 230.0 * dot(m, g);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	fragColor.g = mix(fragColor.r, texture(iChannel0, vec2(xpos + noise * 0.05, uv.y)).g, iMouse.y / iResolution.y);
	fragColor.b = mix(fragColor.r, texture(iChannel0, vec2(xpos - noise * 0.05, uv.y)).b, iMouse.y / iResolution.y);
}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Intensity",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Saturation",       FF_TYPE_STANDARD, 0.25f); m_UserMouseY = 0.5f;
