...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+035",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2DLiquidVoronoi",						// *** Plugin name - make it different for each plugin 
...
// 
// based on https://www.shadertoy.com/view/ldXBDs
//

float roundness = 12.0;
...
float map(vec2 p) {
	...
	for (float i = 1.0; i < 1.+ 11.*inputColour.x; i++) {
		//making cell position
		vec2 orig = vec2(sin(iTime*i*0.1), cos(iTime*i*0.12))*10.0;
	...
	//finding length
	for (float i = 1.0; i < 30.0* iMouse.w / iResolution.y; i++) {
		//making cell position
		vec2 orig = vec2(sin(-1.57+ iTime*i*0.05), cos( iTime*i*0.06))*50.0*iMouse.z / iResolution.x;
		...
		//if (!any(equal(orig, cell)))
		{
			...
			len = smin(len, abs(num), roundness*iMouse.y / iResolution.y);
		}
	}
...
{
	vec2 uv = 130.*(.1 + iMouse.x / iResolution.x)*(fragCoord.xy*2.0 - iResolution.xy) / iResolution.y;
	uv.x += 100.*(.5 - inputColour.y);
	uv.y += 100.*(.5 - inputColour.z);

	//fragColor = vec4(1. - (map(uv)*10.0));
	  fragColor = vec4(1. - 10.*map(uv));
}
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "ZoomOut",       FF_TYPE_STANDARD, 0.05f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Roundness",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Distance",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Lines",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "*Dots",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "Pos X",         FF_TYPE_STANDARD, 0.5f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "Pos Y",          FF_TYPE_STANDARD, 0.5f); m_UserBlue = 0.5f;
