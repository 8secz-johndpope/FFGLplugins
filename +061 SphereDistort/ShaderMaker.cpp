...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+061",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+SphereDistort",						// *** Plugin name - make it different for each plugin 
	...
	"alexnotkin2018", // *** Plugin description - you can expand on this
...
// based on https://www.shadertoy.com/view/4d33RM
// and https://www.shadertoy.com/view/XddBzX

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{

	vec2 offs = vec2(cos(iTime * 0.5), sin(iTime * 0.5));

	vec2 uv = 2.*(.1 + iMouse.x / iResolution.x)*
		(2. * fragCoord - iResolution.xy) / iResolution.y;	// center coordinates

	// scroll sphere around with offset
	uv += iMouse.w / iResolution.y* offs;

	float dist = sqrt(abs(1.0 - iMouse.y / iResolution.y* dot(uv, uv)));

	uv *= .5*(0.2 + iMouse.z / iResolution.x)*(2.0 - dist);
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "Flat/Sphere",       FF_TYPE_STANDARD, 1.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Zoom Texture",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Off/Orbit",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftY = 0.5f;
