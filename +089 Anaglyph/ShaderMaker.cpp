...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+089",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Anaglyph",						// *** Plugin name - make it different for each plugin 
...
// based on https://www.shadertoy.com/view/XdySWm
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	vec2 uv = fragCoord.xy / iResolution.xy;

	vec4 left = texture(iChannel0, uv);
	vec4 right = texture(iChannel0, uv + vec2(0.05*(.5 - iMouse.x / iResolution.x), 0.0));

	vec3 color = vec3(left.r, right.gb);
	color = clamp(color, 0.0 + 0.5*iMouse.y / iResolution.y, 1.0);
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Shift L/R",       FF_TYPE_STANDARD, 0.1f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Bleach",       FF_TYPE_STANDARD, 0.1f); m_UserMouseY = 0.5f;
