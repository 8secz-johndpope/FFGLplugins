...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+112",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+GlitchBlue",						// *** Plugin name - make it different for each plugin 
	...
	"alexnotkin 2018", // *** Plugin description - you can expand on this
	"alexnotkin at gmail.com"			// *** About - use your own name and details
);
...
// based on https://www.shadertoy.com/view/MtfSz2
vec3 rgb2hsv(vec3 c) {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	hsvColor.x = .59;
	hsvColor.y = inputColour.x* clamp(hsvColor.y, .4, .6); //saturation blue

	float noise = rand(1. + fragCoord.y + fragCoord.x * iTime);
	hsvColor.z += noise * .2;

	// vertical lines
	hsvColor.z += .3 * step(mod(fragCoord.x, (0.1 + iMouse.w / iResolution.y)*2.5), 1.);

	// horizontal lines
	float offset = rand(fragCoord.y * fragCoord.x + fragCoord.x * iTime) * 10.;
	float ratio = 2.*iMouse.y / iResolution.y* step(rand(50. + fragCoord.y + fragCoord.x * iTime), .4);
	float timeShift = cos(iTime) * 200.;
	hsvColor.z += .4 * step(mod(iMouse.z / iResolution.x*fragCoord.y + offset + iTime * 400. + timeShift, 200.), 10.) * ratio;
  ...
  //  hsvColor.z += .15 * step(mod(fragCoord.y + iTime + offset2 * 100. + 100., 200.), 100.);
	...
	hsvColor.z += iMouse.x / iResolution.x*  .5 * step(mod(fragCoord.y + iTime * 15000., 2000. + 2000. * offset3), 400. + 800. * offset3);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	printf("Spack-O-Mat Effect Template \n");
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Horiz Rects",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "White Lines",       FF_TYPE_STANDARD, 0.9f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "WLines Number ",  FF_TYPE_STANDARD, 0.3f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Vert Lines",  FF_TYPE_STANDARD, 0.9f); m_UserMouseLeftY = 0.5f;
    SetParamInfo(FFPARAM_RED,           "Saturation",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
