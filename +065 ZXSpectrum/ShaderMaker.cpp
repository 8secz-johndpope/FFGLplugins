...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+065",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+ZxSpectrum",						// *** Plugin name - make it different for each plugin 
...
// based on shader source code from XIX https://www.shadertoy.com/view/XsfcD8
// adjust input camera gamma
const float CAMPOW = 1.0;

// chunky pixel size
const float  LOWREZ = 8.0;
...
void zxspectrum_clash(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec2 bv = floor(pv / 8.0)*8.0;  // pixel size
	...
	for (int py = 1; py < 8; py++)
	{
		...
		{
			//threshold
			vec4 cs = 2.*(0.25 + iMouse.y / iResolution.y)* bmap((texture(iChannel0, (bv + vec2(px, py)) / sv).rgb));
			bright += cs.a;
	...
	{
		//checker amount
		fragColor = iMouse.z / iResolution.x*  vec4(
			dd >= (DITHER*0.5) ? c1.r : c2.r,
			dd >= (DITHER*0.5) ? c1.g : c2.g,
			dd >= (DITHER*0.5) ? c1.b : c2.b,
			1.0);
...
*/
	...
	// SetParamInfo(FFPARAM_MOUSEX,        "LowRezBloxSize",       FF_TYPE_STANDARD, 1.0f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Threshold",       FF_TYPE_STANDARD, 0.24f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Checker ",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftX = 0.5f;
