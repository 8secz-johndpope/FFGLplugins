...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+027",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Chladni",						// *** Plugin name - make it different for each plugin 
...
// based on shader code from https://www.shadertoy.com/view/4dXSD2
...
{
	// Domain [0,1]x[0,1]
	vec2 uv =  2.0 * iMouse.z / iResolution.x * fragCoord.xy / iResolution.y; //contr 3
	...
	vec2 c = (0.01 + iMouse.w / iResolution.x)*  R * vec2(1.0, -1.0);  //    // WIDTH
	...
	u = (0.5+ inputColour.x)*(0.5 + u / 2.0); //contr5
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Shift X ",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Shift Y",       FF_TYPE_STANDARD, 0.15f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Zoom Out",  FF_TYPE_STANDARD, 0.15f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Line Width",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Size",           FF_TYPE_STANDARD, 1.0f); m_UserRed = 0.5f;
