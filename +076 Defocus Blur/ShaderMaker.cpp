...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+076",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+DefocusBlur",						// *** Plugin name - make it different for each plugin 
...
// based on https://www.shadertoy.com/view/MdtXWr
...
{
	float sigma = (1.01 - iMouse.z / iResolution.x)* 25.0 * length(iMouse.xy / iResolution.xy - uv);
...
//	SetParamInfo(FFPARAM_SPEED,         "Speed",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed = 0.5f;
	SetParamInfo(FFPARAM_MOUSEX,        "Left/Right",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Up/Down",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Spot Size",  FF_TYPE_STANDARD, 0.2f); m_UserMouseLeftX = 0.5f;
