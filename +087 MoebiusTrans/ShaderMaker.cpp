...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+087",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+MoebiusTrans",						// *** Plugin name - make it different for each plugin 
...
// Based on https://www.shadertoy.com/view/4dySW3
...
void mainImage(out vec4 O, vec2 U) {
	...
	U = (0.05 + iMouse.x / iResolution.x)* (U + U - z) / -z.y;

	z = U - vec2(-1, 0);  U.x -= .5;                         // Moebius transform
	U *= mat2(z, -z.y, z.x) / dot(U, U);

	// offset   spiral, zoom      phase     // spiraling
	U = log(length(U += .5))*(0.1 + iMouse.y / iResolution.y)*vec2(.5, -.5) + iTime / 8.
		+ (0.1 + iMouse.z / iResolution.x)* atan(U.y, U.x) / 2.3 * vec2(5, 1);
	// n  
	//O += length(fract(U*3.)) -O;
	//O  = texture(iChannel0, fract(U*3.));  // U*1. is also nice
	O = texture(iChannel0, fract(U.yx*3.*(0.01 + iMouse.w / iResolution.y))); // for van dam video
																			  //O += length(sin(U*30.)) -O;
}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "Intensity",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Offset Phase ",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Iterations",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
