...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+140",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+DrosteZoomFX",						// *** Plugin name - make it different for each plugin 
...
// based on the source code from roywig https://www.shadertoy.com/view/Ml33R7
void mainImage(out vec4 c, vec2 z)
{
	z = z / iResolution.xy - .5;

	z = (0.1 + 2.*iMouse.w / iResolution.y)* vec2(log(length(z)),
		atan(z.y, z.x))*mat2(1, .11, -.1, 1); //angle
	c.xy = (0.1 + iMouse.z / iResolution.x)
		*abs(z = exp((0.5 + 2.*iMouse.x / iResolution.x)*z.x
			- mod(iTime*.5, 2.09)) * vec2(cos(z.y), sin(z.y)));
	c = texture(iChannel0, .5 + (0.1 + iMouse.y / iResolution.y)*z*exp2(ceil(-log2(max(c.y, c.x)) - 2.)));
}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "BendEdges",       FF_TYPE_STANDARD, 0.25f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "ZoomTexture",       FF_TYPE_STANDARD, 1.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Multiple/Single",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Angle",  FF_TYPE_STANDARD, 0.45f); m_UserMouseLeftY = 0.5f;
