...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+126",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+VoronoiParticlz",						// *** Plugin name - make it different for each plugin 
...
vec2 rnd2(vec2 s) {
	return  fract(sin(vec2(dot(s, vec2(127.1, 311.7)),
		dot(s, vec2(269.5, 183.3))))*43758.5453);
}

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	float mr = 10.*(0.01 + iMouse.x / iResolution.x)*min(iResolution.x, iResolution.y);
	vec2 uv = (fragCoord*2.0 - iResolution.xy)*S / mr;
	vec2 iuv = floor(uv);
	vec2 fuv = fract(uv);

	float t = fract(iTime)*TWO_PI;
	float md = iMouse.y / iResolution.y;
	vec2 cuv = iuv;
	for (int x = -1; x <= 1; ++x) {
		for (int y = -1; y <= 1; ++y) {
			vec2 n = iMouse.w / iResolution.y* vec2(float(x), float(y));
			vec2 rp = 0.5 + iMouse.z / iResolution.x * sin(t + rnd2(iuv + n)*TWO_PI)*0.5;
	...
	vec2 tuv = mr * 0.5 / S * cuv / iResolution.xy + 0.5;
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Cell Size",       FF_TYPE_STANDARD, 0.04f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Circle Size",       FF_TYPE_STANDARD, 0.9f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Amplitude",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Patternify",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
