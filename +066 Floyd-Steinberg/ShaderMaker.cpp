...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+066",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Floyd-Steinberg",						// *** Plugin name - make it different for each plugin 
...
//based on https://www.shadertoy.com/view/4sjGRD

const int lookupSize = 64;
const float errorCarry = 0.5;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
	...
	if (cornerGapX + cornerGapY < cornerThreshhold) {
		...
		for (int xLook = 0; xLook < lookupSize; xLook++) {
			...
			xError = iMouse.x / iResolution.x* (grayscale - bit)*errorCarry;
		}
		...
		for (int yLook = 0; yLook < lookupSize; yLook++) {
			...
			grayscale += yError;
			float bit = grayscale >= 0.5 ? 1.0 : 0.0;
			yError = iMouse.y / iResolution.y* (grayscale - bit)*errorCarry;
		}

		float finalGrayscale = -.1 + .26*iMouse.z / iResolution.x + getGrayscale(fragCoord.xy);
...
//	SetParamInfo(FFPARAM_SPEED,         "Speed",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed = 0.5f;
	SetParamInfo(FFPARAM_MOUSEX,        "X Error",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Y Error",       FF_TYPE_STANDARD, 0.9f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Threshold ",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
