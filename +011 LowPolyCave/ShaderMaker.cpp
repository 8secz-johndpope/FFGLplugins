...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+011",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+LowPolyCave",						// *** Plugin name - make it different for each plugin 
...
void main() {
	...
	float FOV = 3.141592*3.*(mouse.x + 0.3) / 3.; // FOV - Field of view.
	vec3 forward = normalize(lookAt - camPos)*(mouse.y + 0.1)*2.;
	...
	if (h < e) {
		...
		vec3 ligPos = camPos + vec3(0.0, 0.5, -3.0);
		vec3 lig = normalize(ligPos - lookAt);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "Width",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
