...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+032",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D Drops",						// *** Plugin name - make it different for each plugin 
...
//based on https://www.shadertoy.com/view/4tcyDX
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
	...
	//uv.x *= iResolution.y / iResolution.x;
	 uv.y += (0.5-inputColour.y)*iTime*0.5;  // up-down movement
	 uv.x += (0.5-inputColour.x)*iTime*0.5 ;
	//uv.y +=  sin(iTime *0.05); 
	vec2 st = uv;
	vec3 color = vec3(1.0);

	// Scale
	st *= 20.*(iMouse.x / iResolution.x + 0.01);

	// Tile the space
	vec2 i_st = floor(st);
	vec2 f_st = fract(st);

	float m_dist = (1.01 - iMouse.y / iResolution.y);  // minimun distance
	for (int j = -2; j <= 2; j++) {
		for (int i = -2; i <= 2; i++) {
			...
			// Animate the offset
			offset = 1.*iMouse.z/iResolution.x*(0.5 + 0.5*sin(iTime + 6.2831*offset));
			...
			m_dist = smin(m_dist, dist, 0.9);

		}
	}

	// Draw cells (thanks @shane)
	color = 1.- vec3(smoothstep(0.0, fwidth(m_dist), (0.01+iMouse.w / iResolution.y) * m_dist));
	//color = vec3(m_dist);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "ZoomOut",       FF_TYPE_STANDARD, 0.25f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Size",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Grid/Offset",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Soft/HardEdges",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "*Left/Right",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "*Up/Down",         FF_TYPE_STANDARD, 0.5f); m_UserGreen = 0.5f;
