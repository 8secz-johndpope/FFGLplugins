...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+072",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+OpArtFilter",						// *** Plugin name - make it different for each plugin 
...
//based on shader code by   zachernuk from  https://www.shadertoy.com/view/XsdGD8

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	vec2 uv = (fragCoord.xy / iResolution.xy);
	vec4 tex = texture(iChannel0, uv);
	float vcol = 10.* iMouse.z / iResolution.x *(mod(fragCoord.y / 10.*( iMouse.y /iResolution.y +0.005), 1.) - .45);
	float hCol = 10.* iMouse.w / iResolution.y *(mod(fragCoord.x / 10.*( iMouse.y /iResolution.y +0.005), 1.) - .55);
	float gray = hCol;
	if (tex.r>0.99 * iMouse.x / iResolution.x) //threshold
	{
		gray = vcol;
	}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "Line Width",       FF_TYPE_STANDARD, 0.3f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Blend Horiz ",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Blend Vert",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
/*	SetParamInfo(FFPARAM_RED,           "Width Ver",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
