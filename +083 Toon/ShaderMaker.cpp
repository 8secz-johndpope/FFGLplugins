...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+083",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Toon",						// *** Plugin name - make it different for each plugin 
...
vec2 texel_size = vec2((0.001 + inputColour.x)*56.0, (0.001 + inputColour.x)*56.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float reaction_coordinate = texture(iChannel0, fragCoord).r;	// Use red channel
	...
	fragColor.rgb = iMouse.y / iResolution.y*vec3(1.0, 0.2, iMouse.w / iResolution.y);	// Red
	if (fragColor.a > 0.65) fragColor.rgb = vec3(1.0, 1.0, 1.0);	// White
	else if (fragColor.a > 0.57* iMouse.x / iResolution.x) fragColor.rgb = iMouse.z / iResolution.x *vec3(1.4, 0.8, iMouse.w / iResolution.y);	// Yellow
	fragColor.a = 1.;// float(fragColor.a > 0.0);
}
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "RedLevel",       FF_TYPE_STANDARD, 1.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "YellowLevel ",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "BlueTone",  FF_TYPE_STANDARD, 0.3f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Pixelization",           FF_TYPE_STANDARD, 0.0f); m_UserRed = 0.5f;
