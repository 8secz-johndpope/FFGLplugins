...
#define FFPARAM_MOUSELEFTX  (11)
#define FFPARAM_MOUSELEFTY  (5)
#define FFPARAM_RED         (7)
#define FFPARAM_GREEN       (8)
#define FFPARAM_BLUE        (10)
#define FFPARAM_ALPHA       (9)

#define FFPARAM_VECTOR1_X       (4)
#define FFPARAM_VECTOR1_Y       (3)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+016",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D Liquidoscope",						// *** Plugin name - make it different for each plugin 
...
//made by checkered/an+visuals


// based on shader code from http://glslsandbox.com/e#26468.0
...
vec2 pentaplexify(const vec2 z) {
	...
	for (float i = 0.; i < TWOPI - 0.1; i += TWOPI / 5.) {
		y = cmul(y, z - vec2(cos(i + .1*iTime), sin(i + .1*iTime)));
	}
	return y;
}
...
vec2 infundibularize(in vec2 z) {
	...
	return vec2(inputColour.x*_periodX*(lnz.y) + inputColour.w*_periodY * (lnz.x),
	      	(inputColour.z)*_periodX*(lnz.x) - inputColour.y*_periodY * (lnz.y));
...
vec3 hsv(float h, float s, float v) {
	return v * mix(
		vec3(1.0),
		clamp((abs(fract(iMouse.w/iResolution.y + h + vec3(3.0, 2.0, 1.0) / 3.0)*6.0 - 3.0) - 1.0), 0.0, 1.0),
		inputVector1.z* s);
}

vec4 rainbowJam(in vec2 z) {
	vec2 uv = fract(vec2(z[0] / _periodX, z[1] / _periodY))*vec2((inputVector1.x+.005)*_periodX, (inputVector1.y + .003)* _periodY);
	vec2 iz = floor(uv);
	vec2 wz = uv - iz;
	return vec4(hsv(pow(iz[0] / _periodX, 1.5), 0.9, smoothstep(0.45, 1.0*iMouse.y / iResolution.y, length(wz - vec2(0.5)))), 1.);
}
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{

	fragColor = rainbowJam(infundibularize(pentaplexify(perturbedNewton(16.*(.002 + iMouse.x / iResolution.x)*(fragCoord.xy - .5*iResolution.xy) / iResolution.y)))
		+ TWOPI * iTime*.3* (-iMouse.z / iResolution.x + .5));
}
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "ZoomOut",       FF_TYPE_STANDARD, 0.18f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Shading",       FF_TYPE_STANDARD, 0.45f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "*Forward/Back",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "HueLoop",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Fragments1",           FF_TYPE_STANDARD, 1.0f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "Fragments2",         FF_TYPE_STANDARD, 1.0f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "TimeSlide",          FF_TYPE_STANDARD, 1.0f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "SpinColors",         FF_TYPE_STANDARD, 1.0f); m_UserAlpha = 1.0f;



	SetParamInfo(FFPARAM_VECTOR1_X, "NumOfColors", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_Y, "Fill", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_Z, "BW/Saturat", FF_TYPE_STANDARD, 1.0f);
