...
#define FFPARAM_GREEN       (7)
#define FFPARAM_BLUE        (6)
#define FFPARAM_ALPHA       (9)

#define FFPARAM_VECTOR1_X       (8)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+029",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+PsyPainter",						// *** Plugin name - make it different for each plugin 
...
// based on https://www.shadertoy.com/view/Mt23zz
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec3 d = 1. - vec3(fragCoord, 1.) / iResolution.x;
	d.y -= 2.*(.5- iMouse.y / iResolution.y); //up-down
	d.x -= 0.5 + 3.*(.5- iMouse.z / iResolution.x);// left right
	d.x = abs(d.x)*5.*(.1 + inputColour.w); //wide
	
	vec3 o = d;
	float l =  length(d.xyz)*10.0; //wave
	float a =  atan(d.y, d.x);

	//zoom contr
	o.xy *= 4.* iMouse.x / iResolution.x  * mat2(cos(i + sin(a + i)) + 10.0, sin(i*.5 + a * l)*2.0, -sin(i + a), cos(i*d.z + l) + 10.0); //flow
	for (int i = 0; i < 40; i++) {
		float m = 10.*inputColour.x;// shift*mouse

		/*  #ifdef MIC
		m = texture( iChannel0,vec2(.5,float(i/40))).r;
		#endif  */

		o += f(o + m)*(d) *(1.-inputColour.y); // SofterBlur
	}
	o.z = inputColour.z* length(o*d); // color move
	vec4 c = vec4(sin(i + abs((o - d) + length(o.xy*step(o.z, 20.0 * iMouse.w/iResolution.y ))))*.3 + 0.2+.6*inputVector1.x , 1.0);// 20.0*mouseW color move
	/*    #ifdef GREY
	c.r = c.b+c.g+c.r;
	c.r/=3.0;
	c.r = pow(c.r,2.0);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "In/Out",       FF_TYPE_STANDARD, 0.3f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Up/Down",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Left/Right",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "ColorMo1",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
	
	SetParamInfo(FFPARAM_RED,           "Shift",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "SofterBlur",         FF_TYPE_STANDARD, 0.0f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "ColorMo2",          FF_TYPE_STANDARD, 0.5f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "Stretch",         FF_TYPE_STANDARD, 0.1f); m_UserAlpha = 1.0f;



	SetParamInfo(FFPARAM_VECTOR1_X, "Brightness", FF_TYPE_STANDARD, 0.5f);
/*	SetParamInfo(FFPARAM_VECTOR1_Y, "WaveMod1", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_Z, "WaveMod2", FF_TYPE_STANDARD, 1.0f);
