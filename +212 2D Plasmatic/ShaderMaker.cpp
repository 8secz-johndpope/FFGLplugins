...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+212",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D Plasmatic",						// *** Plugin name - make it different for each plugin 
	...
	"Alexey Notkin 2019", // *** Plugin description - you can expand on this
...
// ==================== PASTE WITHIN THESE LINES =======================
 // based on https://www.shadertoy.com/view/XldGRj
//
//
...
vec3 hsv2rgb(vec3 c) {
	//vec4 K = vec4((iMouse.x / iResolution.x + .1)*5.0, 2.0*iMouse.y / iResolution.y , iMouse.z / iResolution.x, 0.45*iMouse.w / iResolution.y);
	vec4 K = vec4(1.0*(inputVector1.x + .1), inputVector1.y*2.0 / 3.0, inputVector1.z*1.0 / 3.0, 3.0 *inputVector1.w);
	
	//vec4 K = vec4( 5.*(inputVector1.x +.1), inputVector1.y*2., inputVector1.z, .45*inputVector1.w);
...
float getDepth(float n) {
	...
	//step it
	d = floor(d*STEPS* inputColour.x) / STEPS;

	return d;

}

/*Modified code from "Noise Holes" by airtight
 https://www.shadertoy.com/view/XdyXz3
*/

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	vec2 uv = 1.5*(.01 + iMouse.x / iResolution.x)* ( fragCoord - .5*iResolution.xy) / iResolution.y;
	

	//uv.x+= .1*iTime*(.5- inputVector2.x);
	//uv.y+= .1*iTime*(.5 - inputVector2.y);
	uv.x*= iMouse.y / iResolution.y;
	uv.y*= iMouse.z / iResolution.x;
 ...
 //   if (uv.x > 0.5) uv.x = 1.0 - uv.x;
	...
	if (noise < 1.7*iMouse.w / iResolution.y* CUTOFF) {
		col = vec3(0., 0., 0.);//black water
	}
	else {
		...
		//calc HSV color
		float h = inputColour.y + d + 0.5; //hue
		float s = .2+ inputColour.w; //saturation
		float v = 0.2 + 0.5*inputColour.z  + (d*0.6);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	// Parameters
	SetParamInfo(FFPARAM_SPEED,         "Speed",         FF_TYPE_STANDARD, 0.2f); m_UserSpeed = 0.5f;
	SetParamInfo(FFPARAM_MOUSEX,        "ZoomOut",       FF_TYPE_STANDARD, 0.3f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Stretch X",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Stretch Y",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "CutOff",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "ColorNum",           FF_TYPE_STANDARD, 1.0f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "ColorLoop",         FF_TYPE_STANDARD, 0.5f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "Brightness",          FF_TYPE_STANDARD, 1.0f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "Saturation",         FF_TYPE_STANDARD, 0.75f); m_UserAlpha = 1.0f;



	SetParamInfo(FFPARAM_VECTOR1_X, "Palette1", FF_TYPE_STANDARD, 0.75f);
	SetParamInfo(FFPARAM_VECTOR1_Y, "Palette2", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_Z, "Palette3", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_W, "Palette4", FF_TYPE_STANDARD, 1.0f);


/*	SetParamInfo(FFPARAM_VECTOR2_X, "Move X", FF_TYPE_STANDARD, 0.5f);
	SetParamInfo(FFPARAM_VECTOR2_Y, "Move Y", FF_TYPE_STANDARD, 0.5f);
