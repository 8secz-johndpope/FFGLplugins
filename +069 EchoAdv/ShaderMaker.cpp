...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+069",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+EchoAdv",						// *** Plugin name - make it different for each plugin 
...
//https://www.shadertoy.com/view/MttXDM Droste(pure math ver)

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	// Customizable Parameters
	float repeat = 10.0 * (0.2 + iMouse.x / iResolution.x);			// How many times it should be repeated.
	...
	vec2 iter = iMouse.y / iResolution.y* min(fiter, riter); // iter = min(forward UV, reverse UV)
	...
	vec2 uv_mul = iMouse.w / iResolution.y* 1.0 / (vec2(1.0, 1.0) - offset * 2.0);
	...
	vec2 finaluv = (uv - offset - iMouse.z / iResolution.x + 0.5) * uv_mul;
...
//	SetParamInfo(FFPARAM_SPEED,         "Speed",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed = 0.5f;
	SetParamInfo(FFPARAM_MOUSEX,        "Repeats",       FF_TYPE_STANDARD, 1.0f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Shift",       FF_TYPE_STANDARD, 1.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Diagonal ",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "ZoomTxt",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
