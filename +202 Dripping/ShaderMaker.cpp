...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+202",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D Dripping",						// *** Plugin name - make it different for each plugin 
...
//based on shader code by cras from https://www.shadertoy.com/view/XlKfDz
float rand(vec2 n) {
...
{
	vec2 st = -(2. * fragCoord - iResolution.xy) / iResolution.y;	// center coordinates
  
	st.y *= iResolution.y / iResolution.x;
	st *= (0.02 + iMouse.x / iResolution.x)*20.;
	st.x -= iTime;
	st.y += 4.*inputColour.x;
  
	float steps = step(0.2, 2.*(0.15 + 2.*iMouse.y / iResolution.y)*noise(st + iTime - 0.1*iMouse.w / iResolution.y*st.x)* fract(2.*(iMouse.z / iResolution.x + .1)*st.y));
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "Less/More",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Steps",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Time Shift ",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Shift Y",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
