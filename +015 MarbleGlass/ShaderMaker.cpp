...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+015",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+MarbleGlass",						// *** Plugin name - make it different for each plugin 
...
// based on shader code from http://glslsandbox.com/e#29226.0
// PlayingMarble.glsl
...
void main()
{
	vec2 p = 4.*(mouse.x + 0.1)*(gl_FragCoord.xy - resolution*0.5) / resolution.y;
	vec3 ro = roty(vec3(3.), time*0.1 + 2.*mouse.y);
