...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+133",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+R-Dither",						// *** Plugin name - make it different for each plugin 
	...
	"alexnotkin 2019", // *** Plugin description - you can expand on this
...
// based on https://www.shadertoy.com/view/WdjGWy
...
float intensity(ivec2 pixel) {
	...
	return fract(a1 * float(pixel.x) + iMouse.y / iResolution.y* a2 * float(pixel.y));//
}

float dither(float gray, int ng) {
	// Calculated noised gray value
	float noised = (2. / float(ng)) * T(intensity(ivec2(gl_FragCoord.xy))) + 5.*(0.1 + iMouse.x / iResolution.x)*gray - (1. / float(ng));
	// Clamp to the number of gray levels we want
	return floor(float(ng) * noised) / (float(ng) - 1.);
}

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	const int ng = 4; // Number of gray levels to use

	vec2 uv = fragCoord / iResolution.xy;

	// Cheap-mans gray and gamma correction (input assumed as sRGB, but we want linear RGB)
	float gray = dot(pow(texture(iChannel0, uv).rgb, vec3(2.2)), vec3(1.)) / 3.;

	// Output to screen, gamma corrected
	fragColor = vec4(vec3(pow(dither(gray, ng), 1.0 / 2.2)), 1.0);
}
...
//	SetParamInfo(FFPARAM_SPEED,         "Speed",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed = 0.5f;
	SetParamInfo(FFPARAM_MOUSEX,        "Threshold",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Angle",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
/*	SetParamInfo(FFPARAM_MOUSELEFTX,    "mouse Z ",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "mouse W",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
