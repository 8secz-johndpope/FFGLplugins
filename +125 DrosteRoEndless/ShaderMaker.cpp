...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+125",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+DrosteRoEndless",						// *** Plugin name - make it different for each plugin 
...
// https://www.shadertoy.com/view/MsK3DG
// Endless frames
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	uv -= .5;
	if (DROSTE_MODE) {
		uv *= (1. / pow(4., fract((time +iMouse.x/iResolution.x* atans) / 2.)));
	}
	else {
	...
	if (DROSTE_MODE) {
        ...
        tri = (0.05+iMouse.y/iResolution.y)*(vec2(length(uv - .5)))*2.;
	}
	float zoom =(1.2- iMouse.z/iResolution.x)* min(pow(2., floor(-log2(tri.x))), pow(2., floor(-log2(tri.y))));
	float zoom_id = log2(zoom) + (1.- iMouse.w/iResolution.y);
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "DrosteMode/On",       FF_TYPE_STANDARD, 1.0f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Multi/Single",       FF_TYPE_STANDARD, 1.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "ZoomTexture ",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "DiagonalShift",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftY = 0.5f;
