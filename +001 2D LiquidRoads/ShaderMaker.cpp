...
#define FFPARAM_BLUE        (8)
#define FFPARAM_ALPHA       (9)

#define FFPARAM_VECTOR1_X       (10)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+001",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D LiquidRoads",						// *** Plugin name - make it different for each plugin 
...
// ==================== PASTE WITHIN THESE LINES =======================
 //based on http://glslsandbox.com/e#1894.0
...
int getClosestPoint(vec2 o)
{
	...
	for (int i = 1; i <= 2 +int(inputVector1.y*200.); i++)
	{
		float fi = float(i);
		vec2 seed = vec2(fi);
		...
		if (length(p - o) < dist)
		{
			dist = (inputColour.y + 0.01)* length(p - o);
			closest = i;
		}
	}
...
{
	vec2 pos = -(0.01 + 4.*iMouse.x / iResolution.x)*(-1.0 + 2.0 * fragCoord.xy / iResolution.xy); //zoom out 
	pos.x *= (iResolution.x / iResolution.y);
	pos.x -= 12.*(0.5 - iMouse.y / iResolution.y); //posX
	pos.y -= 6.*(0.5 - iMouse.z / iResolution.x); //pos Y
	vec3 color = vec3(0.1, 0.1, 0.1);

	float u = length(pos*(inputColour.x + 0.01));
	float v = atan(pos.y, 12.*(-iMouse.w / iResolution.y + .5) - pos.x); 
	

	float t =  iTime - 1.0 / pos.y;
	if (pos.y > 0.0) t = .25 * iTime + 1.0 / u;

	float fid = float(getClosestPoint(vec2(sin(t) * u, v)));

	color.x = inputColour.z *rand(vec2(fid, fid * 501.0)); //Red
	color.y = inputColour.w *rand(vec2(fid, fid * 511.0)); //Green channel
	color.z = inputVector1.x*rand(vec2(fid, fid * 521.0)); // Blue
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSELEFTX, "Up/Down", FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY, "CollapsePoint", FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED, "Period", FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN, "Density", FF_TYPE_STANDARD, 1.0f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE, "Red", FF_TYPE_STANDARD, 1.0f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA, "Green",  FF_TYPE_STANDARD, 1.0f); m_UserAlpha = 1.0f;



	SetParamInfo(FFPARAM_VECTOR1_X, "Blue", FF_TYPE_STANDARD, 1.0f);
	SetParamInfo(FFPARAM_VECTOR1_Y, "Complexity", FF_TYPE_STANDARD, 0.36f);
