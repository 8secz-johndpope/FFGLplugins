...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+077",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+CyberPunk",						// *** Plugin name - make it different for each plugin 
...
// based on https://www.shadertoy.com/view/Mt2yDm 
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	vec2 uv = fragCoord.xy / iResolution.xy;
	vec4 c = texture(iChannel0, uv);
	fragColor = mod((0.01 + 0.5*iMouse.x / iResolution.x)* iTime* vec4(floatBitsToInt(c)),
		5.);
}
...
*/
	...
	// Parameters
	SetParamInfo(FFPARAM_SPEED,         "Off/Flashing",         FF_TYPE_STANDARD, 0.1f); m_UserSpeed = 0.5f;
	SetParamInfo(FFPARAM_MOUSEX,        "Mod",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
/*	SetParamInfo(FFPARAM_MOUSEY,        "Glitch",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
