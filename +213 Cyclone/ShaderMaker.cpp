...
#define FFPARAM_MOUSEY      (4)
#define FFPARAM_MOUSELEFTX  (5)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+213",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D Cyclone",						// *** Plugin name - make it different for each plugin 
...
/* https://www.shadertoy.com/view/3lsGRB
...
float grid(vec2 p, float res, float k) {
	p.y += (iMouse.y / iResolution.y - .5)*.2* iTime*k;
	p.x += (iMouse.z / iResolution.x - .5)*.2* iTime*k;
	vec2 u = fract(p * res) - .5;
	vec2 e = floor(p * res) - .5;
	float r = random2(e).x * 0.25;
	vec2 rnd = random2(floor(p.y + 2.) + e) - 0.5;
	u -= rnd * (1.0 - r * 2.0);
	float d = length(u / .1);
	return  smoothstep(r, r*1.05, .5*(1.2 - inputColour.x)* d); //dot size

}

void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	vec2 uv = 0.5*(.01 + iMouse.x / iResolution.x)* (fragCoord.xy - 0.5 * iResolution.xy) / iResolution.y;

	float z = 1.0;
	for (float i = 3.; i < (.4 + iMouse.w / iResolution.y)* 10.0; i++) {
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEY,        "*SpinDown/Up",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "*SpinLeft/Right",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Depth",  FF_TYPE_STANDARD, 0.7f); m_UserMouseLeftY = 0.5f;
    SetParamInfo(FFPARAM_RED,           "DotSize",           FF_TYPE_STANDARD, 1.0f); m_UserRed = 0.5f;
