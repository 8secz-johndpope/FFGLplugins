...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+111",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Glitcher3 ",						// *** Plugin name - make it different for each plugin 
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float time = floor(iTime * SPEED * 20.0);
	...
	for (float i = 0.0; i < 20.0 * AMT * iMouse.x / iResolution.x; i += 1.0) {
		float sliceY = random2d(vec2(time, 2345.0 + float(i)));
		float sliceH = random2d(vec2(time, 9035.0 + float(i))) * 0.25;
		float hOffset = randomRange(vec2(time, 9625.0 + float(i)), -maxOffset, maxOffset);
		vec2 uvOff = uv;
		uvOff.x += (0.05 + iMouse.y / iResolution.y)*  hOffset;
	...
	float maxColOffset = iMouse.z / iResolution.x* AMT / 4.0;
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Amount",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Pix Offset",       FF_TYPE_STANDARD, 1.0f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Col Offset",  FF_TYPE_STANDARD, 0.1f); m_UserMouseLeftX = 0.5f;
