...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+110",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+GlitchDist",						// *** Plugin name - make it different for each plugin 
...
// based on shader code by Berkin from
// https://www.shadertoy.com/view/MscGzl

float GlitchAmount = 1.5;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float amount = pow(GlitchAmount *(0.1 + iMouse.x / iResolution.x), 2.0);
	vec2 pixel = iMouse.y / iResolution.y* 1.0 / iResolution.xy;
	...
	float t = mod(mod(iTime*.001, amount * 100.0 * (amount - 0.5)) * 109.0, 1.0);
...
*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Distort",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "PixShift",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
