...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+049",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+TrafficLines",						// *** Plugin name - make it different for each plugin 
...
// based on shader code "Scrolling Dashed Lines" by cacheflowe
// https://www.shadertoy.com/view/llyXWh
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec2 uv = iMouse.x / iResolution.x *(2. * fragCoord - iResolution.xy) / iResolution.y;//Zoom Contrl	// center coordinates
	float rowThickness = uv.y * iMouse.y / iResolution.y * 20.;	//wide							// break y lines up
	float rowIndex = (0.1 + 0.9 * iMouse.z/iResolution.x) *floor(rowThickness); //Cut levels							// break y lines up & get current index
	...
	uv *=  iMouse.w / iResolution.y * vec2(dashLength * 7., 1.);	// length CONTR4							// calc dash lengths by multiplying x
	float timeAdd = (mod(rowIndex, 2.) == 0.) ? time : -time;		// move x in different directions
	timeAdd *= 5.1;													// increase x movement
	float xOffset = inputColour.x * rowIndex / 3.;		//CONTR5 offset							// give lines x offset so they don't line up
	uv += vec2(timeAdd + dashLength + xOffset, 0.);					// move x position
	float col = 0.;													// default black
	if (fract(uv.x) > 0.5 *inputColour.y) //gap contrl
	{											// dash    
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Zoom",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Wide",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Cut Levels",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "length",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Offset",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "Gap",         FF_TYPE_STANDARD, 0.5f); m_UserGreen = 0.5f;
