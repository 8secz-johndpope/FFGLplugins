...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+080",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+Tilt-Shift",						// *** Plugin name - make it different for each plugin 
...
// Based on https://www.shadertoy.com/view/ldyXRm
//#ifdef GL_ES
//precision mediump float;
//#endif
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	//   tcoord.y -= iMouse.y/iResolution.y;

	if (fragCoord.y < iMouse.z)
	{
		...
		amount = pow((tcoord.y * center)*(0.15 + iMouse.y / iResolution.y) * 3.0 - 1.0, 2.0)
			* bluramount* 2.*iMouse.x / iResolution.x;
...
// SetParamInfo(FFPARAM_SPEED,         "Speed",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed = 0.5f;
	...
	SetParamInfo(FFPARAM_MOUSEY,        "Move Y",       FF_TYPE_STANDARD, 0.55f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "CutOff",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftX = 0.5f;
