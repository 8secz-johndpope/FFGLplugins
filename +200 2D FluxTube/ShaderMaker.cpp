...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+200",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D FluxTube",						// *** Plugin name - make it different for each plugin 
...
// based on shader code by samlo from https://www.shadertoy.com/view/wsSSRD
...
vec3 palette(in float t) {
	...
	vec3 d = vec3(0.3, 0.1, 0.85);

	return a + b * cos(6.28318 * (c * t + d));
}

float sin01(in float t) {
	return (sin(t) + 1.) * 0.5; //color slide
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec2 center = 5.*(0.1)* vec2(0.5 - (sin(0.5) * 0.1), 0.5 - (sin(0.25) * 0.1));
	//vec2 uv = (2. * fragCoord - iResolution.xy) / iResolution.y;	// center coordinates
		//aspect ratio
	float aspect = iResolution.x / iResolution.y;
	point.x *= aspect;
	center.x *= 3.*(-0.5 + 2.5*iMouse.y / iResolution.y)* aspect;
	center.y *= 3.*(-0.5 + 2.5*iMouse.x / iResolution.x)* aspect;

	//center circle
	//float circle = pow(smoothstep(0.3, 0., length(point - center)), 1.2) * 0.5;//55;
	
	//float circle = pow(smoothstep(0.03, 0.02, length(point - center)), 1.2) * 0.75;
	...
	//planar distortion
	float rInv = 1.*(0.05 + inputColour.y) / length(point - center); //density
	//float rInv = 1. / length(point - center); //density
	point = point * rInv - vec2(rInv, 0.0);

	//sample noise
	vec2 uv = vec2(point.x, point.y - 3.*iTime * (0.5 - iMouse.z / iResolution.x)) ;  // backward/forward
	//float noise = iqnoise(uv, 0.5*iMouse.w / iResolution.y ,  1.2*iMouse.w / iResolution.y);
	float noise = iqnoise(uv, (iMouse.w / iResolution.y)-0.01, (iMouse.w / iResolution.y)-0.01); //0.5, 1.2
	float stepNoise = floor((0.1 + inputColour.z)* noise * 10.3) / (1. + 5.*inputColour.w); //stepping irregularly gives nice shapes   contr
	...
	vec3 outlineCol = outline * palette(sin01(0.2));
	...
	vec3 palette = 2.*(0.1 + inputColour.x)* palette(stepNoise * 0.5); //brightness

	//combine
	vec3 finalColor = palette + outlineCol;// +circle;// +glow;
	//finalColor = vec3( circle);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Up/Down",       FF_TYPE_STANDARD, 0.33f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Left/Right",       FF_TYPE_STANDARD, 0.5f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "*Back/Forward",  FF_TYPE_STANDARD, 0.7f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Squarify/Liquify",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Brightness",           FF_TYPE_STANDARD, 0.45f); m_UserRed = 0.5f;
	SetParamInfo(FFPARAM_GREEN,         "PlanarDist",         FF_TYPE_STANDARD, 1.0f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "ColorsNumber",          FF_TYPE_STANDARD, 1.0f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "Palette",         FF_TYPE_STANDARD, 0.85f); m_UserAlpha = 0.5f;
/*
