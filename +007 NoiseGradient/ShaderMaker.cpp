...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+007",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+NoiseGradient",						// *** Plugin name - make it different for each plugin 
...
// based on shader code from http://glslsandbox.com/e#40662.1
...
{
	vec2 p = (0.01+iMouse.x/iResolution.x) * fragCoord.xy / min(iResolution.x, iResolution.y);

	float v = rand(p + vec2(fract(iTime)));

	fragColor = vec4(vec3(step(v, p.y)), 1.0);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Gradient",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
