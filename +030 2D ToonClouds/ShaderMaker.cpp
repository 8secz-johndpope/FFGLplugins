...
static CFFGLPluginInfo PluginInfo ( 
	...
	"+030",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"+2D ToonClouds",						// *** Plugin name - make it different for each plugin 
...
// ----------------------------------------------------------------------------------------
// based on shader code from  http://glslsandbox.com/e#35189.5
// 
// ----------------------------------------------------------------------------------------
//	"Toon Cloud" by Antoine Clappier - March 2015
//  originally taken from https://www.shadertoy.com/view/4t23RR
//	Licensed under:
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	// UV.x-=(0.5-iMouse.x/iResolution.x)*0.1*iTime ;
	//uv.y *= -1.;
	// Render:
	...
	{
		// Cloud Layer: 
		float Lt = iTime*(0.5 + 2.0* J)*((-0.5 + iMouse.x / iResolution.x) + 0.1*sin(226.0*J)) + 17.0*J; //shift LR
						//	float Lt =  iTime*(0.5  + 2.0*J)*(1.0 + 0.1*sin(226.0*J)) + 17.0*J; //shift LR
		vec2 Lp = vec2(0.0, 0.5-2.*inputColour.x + 2.*(0.3 + iMouse.y / iResolution.y)* (J - 0.5)); // upper
		float L = Layer(UV + Lp, Lt);

		// Blur and color:
		float Blur = iMouse.w/iResolution.y* 4.0*(0.5*abs(2.0 - 5.0*J)) / (11.0 - 5.0*J);

		float V = mix(0.0, 1.0, 1.0 - smoothstep(0.0, 0.01 + 0.2*Blur, L));
		vec3 Lc = mix(CloudColor, vec3( iMouse.z / iResolution.x), J);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Left/Right",       FF_TYPE_STANDARD, 1.0f); m_UserMouseX = 0.5f;
	SetParamInfo(FFPARAM_MOUSEY,        "Parallax",       FF_TYPE_STANDARD, 0.3f); m_UserMouseY = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Color",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftX = 0.5f;
	SetParamInfo(FFPARAM_MOUSELEFTY,    "Blur",  FF_TYPE_STANDARD, 1.0f); m_UserMouseLeftY = 0.5f;
	SetParamInfo(FFPARAM_RED,           "Up/Down",           FF_TYPE_STANDARD, 0.0f); m_UserRed = 0.5f;
